...
public class MultiColPaginatedMenuActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_multicol_paginated_menu);
    ...
    public void previousProposals(View view) {
        // Select each button and replace its text with the 8th item before it
        for(int i = 1 ; i<=8 ; i++) {
    ...
    public void nextProposals(View view) {
        // Select each button and replace its text with the 8th item after it
        for(int i = 1 ; i<=8 ; i++) {
            String string_id = "id_item" + i;
            int button_id = getResources().getIdentifier(string_id, "id", getPackageName());
            Button button = (Button) findViewById(button_id);
        ...
        // Add previous option (not the first set of 6 items)
        Button button = (Button) findViewById(R.id.id_item_previous);
